...
@benutzer_bp.route('/<int:id>/reset-passwort', methods=['POST'])
def reset_passwort(id):
    """Reset user password to a generated one."""
    user = User.query.get_or_404(id)
    ...
    log_mittel('system', 'benutzer_passwort', f'Passwort für {user.full_name} zurückgesetzt')

    flash(f'Neues Passwort für {user.full_name}: {new_password}', 'info')
